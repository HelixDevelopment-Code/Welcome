...
# GithubPagesWebsite per CONST-035 + CONST-050(B). Cascade per CONST-051(A).

set -uo pipefail
REPLICAS="${GITHUBPAGES_SCALING_REPLICA_URLS:-}"
REQS="${SCALING_REQS_PER_REPLICA:-50}"
CONC="${SCALING_CONCURRENCY:-10}"
MIN_PCT="${SCALING_MIN_PASS_PCT:-95}"

echo "=== GithubPagesWebsite Scaling Challenge ==="
echo "  replicas=$REPLICAS reqs=$REQS conc=$CONC pass≥${MIN_PCT}%"

if [[ -z "$REPLICAS" ]]; then
    echo "[1/6] SKIP: GITHUBPAGES_SCALING_REPLICA_URLS unset — SKIP-OK: #env-single-replica"
    echo "=== GithubPagesWebsite Scaling Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== GithubPagesWebsite Scaling Challenge: PASSED ==="
